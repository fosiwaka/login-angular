...
import { DeleteParamas, ErrorServicio, GetParams, HeadersConfig, PostParams, PutParams, ResponseData } from 'src/app/interfaces/http-request.interface';
...
export class HttpRequestService {
  ...
  getHeaders(): HeadersConfig {
    ...
    // const dominio = `${env.MAIN_URL}${env.PORT}/api`;
    const dominio = '';
    const headersConf = {
  ...
  get(requestParams: GetParams): Observable<any> {
    ...
    const urlComplete = requestParams.isPublicRoute
      ? requestParams.endpoint
      : `${dominio}${requestParams.endpoint}`;
    return this.http
      .get(urlComplete, {
        headers,
  ...
  processError(error: ErrorServicio) {
    const httpMessageError = {
      errorProtocolo:
        'Ocurrió un error con la conexión, por favor, verifica la conexión de internet e intentalo de nuevo',
    };
    const errorProcesado = error.error?.mensajeError
      ? error.error?.mensajeError
      : httpMessageError.errorProtocolo;
    ...
    if (params) {
      ...
      Object.keys(params).forEach((key) => {
        if (Array.isArray(params[key])) {
          params[key].forEach(
            (value: any) => (httpParams = httpParams.append(key, value))
          );
